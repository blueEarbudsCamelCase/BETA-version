...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (addTaskBtn)
    addTaskBtn.onclick = () => document.getElementById('addTaskPopup').classList.remove('hidden');
  if (cancelTaskBtn)
      ...
      for (const row of rows) {
        ...
        if (label === currentPeriodLabel) {
          ...
          if (fallbackTd) {
            ...
            console.debug('[schedule] Highlighted current period via fallback', {
              tzKey,
              currentPeriodLabel,
              dayIndex,
            });
  ...
  saveBtn.onclick = () => {
    ...
    if (updated) {
      ...
      taskElements.forEach((el) => {
        ...
        if (summarySpan)
          summarySpan.textContent = `${newTitle} - ${el.dataset.estimatedTime || ''} min.`;
      });
    } else {
...
function openMapPopup() {
  ...
  timeButtons.forEach((btn) => {
    btn.onclick = () => {
      ...
      console.log('[Time Selected]', mapTimeSelected, 'Zone Selected', mapZoneSelected);
      tryMapAutoSave();
    };
  });
  ...
  zoneBtns.forEach((btn) => {
    ...
    btn.onclick = () => {
      ...
      if (mapSelectedZone === 'Collaborative')
        btn.classList.add('ring', 'ring-offset-2', 'ring-red-300');
      console.log('[Zone Selected]', mapZoneSelected, 'Time Selected', mapTimeSelected);
      tryMapAutoSave();
    };
  });
